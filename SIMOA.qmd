...
```{r}
#| label: mean/sd for age (All Respondents)

# Ensure the 'What is your age?' column is numeric (in case it's not already)
SIMOA$`What is your age?` <- as.numeric(SIMOA$`What is your age?`)

# Filter the data to include only people who are ≥65 and answered the sleeping pill question
AVG_AGE_AR <- SIMOA %>%
  filter(
...
# Calculate mean, standard deviation, and the number of people
mean_age <- mean(AVG_AGE_AR$`What is your age?`, na.rm = TRUE)
sd_age <- sd(AVG_AGE_AR$`What is your age?`, na.rm = TRUE)
n_people <- nrow(AVG_AGE_AR)
